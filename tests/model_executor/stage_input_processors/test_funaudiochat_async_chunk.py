...
def test_first_chunk_fires_fast_with_zero_offset_and_switches_to_steady_hop():
    tm = _transfer_manager()
    req = _request("r1")
    out = funaudiochat2code2wav_async_chunk(tm, _mm(torch.tensor([5] * (_INITIAL_HOP + _PRE), dtype=torch.long)), req)
...
def test_second_chunk_uses_steady_state_hop_and_offset_equals_initial_hop():
    tm = _transfer_manager()
    req = _request("r1")
    funaudiochat2code2wav_async_chunk(tm, _mm(torch.tensor([5] * (_INITIAL_HOP + _PRE), dtype=torch.long)), req)
    ...
    assert funaudiochat2code2wav_async_chunk(tm, _mm(torch.tensor([5] * (_HOP - 1), dtype=torch.long)), req) is None
...
def test_finish_flushes_remaining_codes_including_lookahead_without_loss():
    ...
    out = funaudiochat2code2wav_async_chunk(tm, _mm(torch.tensor([5] * 10, dtype=torch.long)), req, is_finished=True)
    assert out is not None
...
def test_placeholder_steps_are_dropped_and_do_not_block_threshold():
    ...
    out = funaudiochat2code2wav_async_chunk(
        tm,
        _mm(torch.tensor([[-1] * (_INITIAL_HOP + _PRE), [5] * (_INITIAL_HOP + _PRE)], dtype=torch.long)),
        req,
    )
    assert out is not None
    assert out.codes.audio.tolist() == [5] * (_INITIAL_HOP + _PRE)
...
def test_no_connector_uses_defaults():
    ...
    tm = _transfer_manager()
    req = _request("r1")
    out = funaudiochat2code2wav_async_chunk(tm, _mm(torch.tensor([5] * (_INITIAL_HOP + _PRE))), req)
    assert out is not None
