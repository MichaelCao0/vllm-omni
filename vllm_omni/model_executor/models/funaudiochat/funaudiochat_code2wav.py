...
class FunAudioChatCosyVoice3Code2Wav(nn.Module):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        # EngineArgs may remove its temporary HF config directory once it has
        # built ModelConfig. Reuse the resolved config, including overrides.
        ...
        # The platform may select FlashAttention before this FP32 conversion.
        # Opt this flow stack into the shared dtype-aware SDPA fallback.
        for module in self.code2wav.flow_model.modules():
            if isinstance(module, Attention):
                module.allow_fp32_fallback = True
        ...
        # Per-request bounded mel context and harmonic phase are owned by
        # the shared CosyVoice3 streaming HiFT implementation.
        ...
        logger.info(
            ...
            "original_noise=true original_attention=true phase_carry=true hift_window=%d",
            self.code2wav._hift_window_len,
        )
    ...
    def _resolve_model_path(self, model_path: str) -> str:
        ...
        return resolve_model_to_local_path(model_path, allow_download=True)
    ...
    def _run_hift_like_official(
        ...
        *,
        ...
        source, _, _, _ = hift.m_source(source)
    ...
    def _run_streaming_hift_chunk(
        ...
        *,
        ...
        """Preserve phase/F0 context and release the shared HiFT lookahead tail.

        Measuring an overlap separately in final and non-final mode changes
        its duration by the F0/HiFT lookahead, dropping the final 8 mel frames.
        The shared path carries the absolute noise offset and harmonic phase
        and accounts for those withheld frames exactly once at finalization.
        """
        return self.code2wav._stream_hift_from_feat(chunk_mel, cache_state=cache_state, finalize=finalize)
    ...
    def _decode_request_streaming(
        ...
        *,
        ...
        logger.debug(
            "FAC stage1 bounded Flow: req=%s token_len=%d token_offset=%d segment_start=%d cache_hit=%s finalize=%s",
            req_id,
            int(token.numel()),
            token_offset,
            segment_start,
        ...
        **_: Any,
        ...
        for idx in range(num_reqs):
            ...
            uses_streaming = raw_meta is not None and (
                (isinstance(raw_meta, dict) and ("stream_finished" in raw_meta or "left_context_size" in raw_meta))
