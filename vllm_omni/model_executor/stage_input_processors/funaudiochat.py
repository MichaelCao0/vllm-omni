...
def funaudiochat2code2wav(
    ...
    for output_wrapper in source_outputs:
        ...
        if filtered.numel() == 0:
            ...
            logger.debug("FunAudioChat stage0->stage1: filtered audio tokens empty; skipping code2wav.")
            continue
...
def funaudiochat2code2wav_async_chunk(
    ...
    if not isinstance(req_state_entry, dict) or _FAC_ASYNC_STATE_KEY not in req_state_entry:
        steady_hop, initial_hop, pre_lookahead_len, flow_segment_len = _resolve_chunk_cfg(transfer_manager)
    ...
    code_predictor_codes = [int(frame) for frame in token_frames[local_segment_start:local_prefix_len]]
    ...
    if not finished:
        ...
        next_segment_start = (next_emitted_token_len // flow_segment_len) * flow_segment_len
