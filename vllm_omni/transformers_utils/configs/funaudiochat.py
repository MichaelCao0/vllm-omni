...
"""FunAudioChat config registration.

The public Fun-Audio-Chat checkpoints use ``model_type="funaudiochat"``.
Depending on the runtime environment, that type may not be known to the
installed Transformers build. Register it eagerly so vLLM-Omni can load the
checkpoint config before the model wrapper is instantiated.
"""
...
try:
    from funaudiochat.configuration_funaudiochat import (  # type: ignore
        FunAudioChatAudioEncoderConfig,
        FunAudioChatConfig,
    )
except ImportError:

    class _FallbackFunAudioChatAudioEncoderConfig(PretrainedConfig):
    ...
    class _FallbackFunAudioChatConfig(PretrainedConfig):
        ...
        def __init__(
            ...
            **kwargs,
            ...
            if isinstance(audio_config, dict):
                audio_config.setdefault("model_type", FunAudioChatAudioEncoderConfig.model_type)
    ...
    FunAudioChatAudioEncoderConfig = _FallbackFunAudioChatAudioEncoderConfig
    FunAudioChatConfig = _FallbackFunAudioChatConfig


AutoConfig.register(FunAudioChatAudioEncoderConfig.model_type, FunAudioChatAudioEncoderConfig, exist_ok=True)
